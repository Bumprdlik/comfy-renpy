...
| `renpy/event` | Oranžová | žádné | `id`, `location_id`, `trigger`, `trigger_label`, `prerequisite`, `time`, `repeatable`, `priority`, `notes`, `body_text?`, `quest_action?`, `quest_id?`, `gives_item?` |
...
Tlačítko **✨ AI dialogy** (`#batch-gen-btn`) je viditelné jen pokud je nastaven API klíč (`setBatchGenVisible(hasAi)` v `main.ts`). Spustí `batchGenerateDialogues()` z `generate.ts` — vygeneruje dialogy sekvenčně pro všechny eventy bez `body_text`.
...
### AI generování dialogů

**Single-event flow**: klik ✨ Generovat dialog na Event uzlu otevře `gen-overlay` s vygenerovaným promptem. Klik Spustit AI zavolá `apiGenerateDialogue(prompt)` → server → Anthropic/OpenAI. Výsledek se uloží do `node.properties['body_text']` přes `writeGenResult()` (scheduleSave + auto-close po 800ms). Při exportu se `body_text` zapíše do `kind=body` markeru.

**Batch flow**: tlačítko ✨ AI dialogy (viditelné jen s API klíčem) zavolá `batchGenerateDialogues()` — sekvenčně vygeneruje dialogy pro všechny eventy bez `body_text`.

**Strukturované quest/item pole** na Event uzlu (pro AI prompt):
- `quest_action` (`none` / `start` / `advance`) — jaký quest helper call má AI vložit
- `quest_id` — ID questu pro `quest_action`
- `gives_item` — ID itemu, pro který AI vloží `$ comfy_give("id")`

`buildPrompt(nodeId)` v `src/ui/modals/generate.ts` — sestaví prompt zahrnující: lokaci, event, postavy, itemy, **katalog všech questů a itemů v grafu**, **referenci helper funkcí**, **strukturované pokyny** z quest_action/quest_id/gives_item, **existující body_text** (pro revizi).

`POST /api/write-dialogue` — starý endpoint (přímý zápis do .rpy) — stále existuje, ale primární flow jde nyní přes body_text → export.
